...
func Test_State_IsReadyToAddNote(t *testing.T) {
	testCases := []struct {
		...
		{
			Name: "audio field not exists",
			State: &State{
				DeckExists:       true,
				NoteTypeExists:   true,
				NoteHasAllFields: true,
				OrderDefined:     true,
				AudioFieldExists: false,
			},
			Expected: false,
		},
	}
...
func Test_statefullClient_getState(t *testing.T) {
	testCases := []struct {
		...
		{
			Name: "audio field required but missing",
			Config: &Config{
				NoteType:   "testnote",
				AudioField: "testaudio",
			},
			Permissions: &ankiconnect.RequestPermissionResponse{
				Permission: ankiconnect.PermissionGranted,
			},
			ModelNames:      []string{"testnote"},
			ModelFieldNames: []string{"key1", "key2"},
			Expected: &State{
				AnkiState: AnkiState{
					NoteTypes: []string{"testnote"},
					NoteFields: map[string][]string{
				...
				NoteTypeExists:   true,
				CurrentFields:    []string{"key1", "key2"},
				NoteHasAllFields: true,
				AudioFieldExists: false,
			},
			AssertError: assert.NoError,
		...
		{
			Name: "audio field required and provided",
			Config: &Config{
				NoteType:   "testnote",
				AudioField: "testaudio",
			},
			...
			ModelNames:      []string{"testnote"},
			ModelFieldNames: []string{"key1", "key2", "testaudio"},
			Expected: &State{
				AnkiState: AnkiState{
					NoteTypes: []string{"testnote"},
					NoteFields: map[string][]string{
						"testnote": {"key1", "key2", "testaudio"},
					},
				},
				NoteTypeExists:   true,
				CurrentFields:    []string{"key1", "key2", "testaudio"},
				NoteHasAllFields: true,
				AudioFieldExists: true,
			},
			AssertError: assert.NoError,
